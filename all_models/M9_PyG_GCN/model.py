"""
Minimal GCN link prediction model for the Science4Cast-style semantic graph.

This module trains a 2-layer GCN on node degree features and performs
dot-product decoding for link prediction on the provided
unconnected_vertex_pairs. It is intentionally dependency-light (pure PyTorch)
so it can run in the existing environment without torch_geometric.

Typical usage
-------------
from all_models.M9_PyG_GCN.model import run_link_prediction
result = run_link_prediction(
    data_path="SemanticGraph_delta_1_cutoff_25_minedge_1.pkl",
    config={"epochs": 30}
)
print(result["auc_val"])
"""
...
# Avoid interactive backends during CLI use.
...
def build_adj(edge_array: np.ndarray, cutoff_year: int, num_nodes: int) -> torch.Tensor:
    """
    Build a symmetric, self-looped, normalized adjacency for GCN.
    edge_array: np.ndarray of shape (E, 3) with [v1, v2, t_day].
    cutoff_year: include edges up to Dec 31 of cutoff_year.
    """
    ...
    if edges.size == 0:
        raise ValueError("No edges found before cutoff; cannot build graph.")
    ...
    edges_all = np.vstack([edges_all, loop_edges])

    edge_index = torch.from_numpy(edges_all.T)  # [2, E]
...
def build_features(adj: torch.Tensor) -> torch.Tensor:
    """Use simple degree feature (one-dimensional) normalized to [0,1]."""
...
class GCNLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, adj: torch.Tensor) -> torch.Tensor:
        # Sparse matrix multiplication: (N, N) x (N, F) -> (N, F)
...
class TrainConfig:
    ...
    max_pairs: int = 200_000  # optional cap to keep runtime manageable
    seed: int = 42


def compute_auc(scores: np.ndarray, labels: np.ndarray) -> float:
    """AUC computed similarly to utils.calculate_ROC but without plotting."""
    ...
    # Approximate area with trapezoidal rule.
...
def train_one_dataset(
    ...
    # Optionally cap pairs for faster runs (random subset so we don't accidentally
    # take a biased prefix from the dataset).
    ...
    train_pairs, train_labels, val_pairs, val_labels = split_pairs(
        pairs, labels, config.train_ratio, config.seed
    )
    ...
    # Handle heavy class imbalance (common in link prediction) with pos_weight.
    ...
    print(
        f"[Setup] device={device} pairs={len(pairs)} "
        f"train_pos_rate={float(np.mean(train_labels)) if len(train_labels) else 0.0:.4f} "
        f"val_pos_rate={float(np.mean(val_labels)) if len(val_labels) else 0.0:.4f}"
    )
    ...
    for epoch in range(config.epochs):
        ...
        print(f"[Epoch {epoch+1}/{config.epochs}] loss={avg_loss:.4f} val_auc={auc:.4f}")

    # Evaluate using best checkpoint (prevents late-epoch collapse from hurting final scores).
    ...
    # Full-pairs evaluation (used for competition-style ranking).
    with torch.no_grad():
...
def load_dataset(data_path: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, int, int, int, int]:
