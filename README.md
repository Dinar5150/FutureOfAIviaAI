...
## Result of M9 (GCN, pure PyTorch)

*Area under the Curve (AUC) for prediction of new edge_weights of 1 (AUC_full)*

|           | *cutoff=0*  | *cutoff=5* | *cutoff=25* |
|-----------|-------------|------------|-------------|
| *delta=1* | 0.8816      | 0.8905     | 0.8897      |
| *delta=3* | 0.8315      | 0.8748     | 0.8650      |
| *delta=5* | 0.7511      | 0.8356     | 0.8343      |

*Area under the Curve (AUC) for prediction of new edge_weights of 3 (AUC_full)*

|           | *cutoff=0*  | *cutoff=5* | *cutoff=25* |
|-----------|-------------|------------|-------------|
| *delta=1* | 0.9563      | 0.9983     | 0.9987      |
| *delta=3* | 0.9601      | 0.9619     | 0.9863      |
| *delta=5* | 0.8924      | 0.9553     | 0.9390      |

Results generated by `python all_models/M9/evaluate_model.py` (see `AUC Summary M9.txt`).

### M9 Implementation Notes (Assignment Report)

**What was implemented**
- A minimal 2-layer GCN link predictor in `all_models/M9/model.py` (pure PyTorch, no `torch_geometric`).
...
- Metric: ROC-AUC computed by calling the repo’s `utils.calculate_ROC` (same function as other models; plotting is disabled during training runs).

**Default hyperparameters (used for the tables above)**
- `epochs=10`, `lr=3e-4`, `hidden_dim=64`, `batch_size=2048`, `weight_decay=1e-4`
- `max_pairs=200000` (random subsample for speed), `train_ratio=0.8`, `seed=42`

**Challenges and fixes**
- Extreme class imbalance (very low positive rate): use `pos_weight = #neg/#pos` inside the loss.
- Unstable/degenerate validation splits for rare positives (e.g., some `minedge=3` settings): report `AUC_full` on the full evaluated pair set as the main number (and keep `AUC_val` as auxiliary).
...
- For `edge_weight=1`, results are competitive but more sensitive to `delta`/`cutoff` and the limited feature set (degree-only features).
